...
async function getRecipes() {
  ...
  //console.log('local: '+localStorage.getItem('recipes'));
  ...
  return new Promise(async function(resolve, reject) {   
    ...
    for(let i=0; i<RECIPE_URLS.length; i++){
      ...
      try {
        ...
        //console.log(`url: ${RECIPE_URLS[i]}`);
        let fetchedRecipe = await fetch(RECIPE_URLS[i]);
        //console.log(`response: ${fetchedRecipe}`);
        ...
        //            "await" again
        let fetchedRecipeJson = await fetchedRecipe.json();
        //console.log(`response json: ${JSON.stringify(fetchedRecipeJson)}`);
        // A8. TODO - Add the new recipe to the recipes array
        fetchedRecipesArray.push(fetchedRecipeJson);
